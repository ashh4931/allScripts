...
namespace SpriteFracture
{
    ...
    public class SpriteFracturer2D : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            // Yerçekimini kapatıyoruz (parçalanana kadar sabit dursun diye)
            rb.gravityScale = 0;
            rb.freezeRotation = true;

            // Fizik tipini belirliyoruz
            // Eğer mod Collision ise objenin çarpabilmesi için Dynamic olmalı.
            // Manual veya diğer modlarda objenin havada asılı kalması/sabit durması için Kinematic en iyisidir.
            rb.bodyType = (triggerMode == TriggerMode.Collision)
                ? RigidbodyType2D.Dynamic
                : RigidbodyType2D.Kinematic;

            // Collider tetikleyici mi olacak?
            if (col != null)
            {
                col.isTrigger = (triggerMode == TriggerMode.Trigger);
            }

            // --- TETİKLEME KONTROLLERİ ---

            if (triggerMode == TriggerMode.Manual)
            {
                // Manual modda hiçbir otomatik işlem başlatma, sadece bekle.
                return;
            }
        ...
        public IEnumerator Fracture()
        {
            ...
            for (int y = 0; y < rows; y++)
            {
                for (int x = 0; x < columns; x++)
                {
                    ...
                    // Yatay (sağ-sol) kuvveti artırmak için x değerini güçlendiriyoruz
                    dir.x *= 2.5f; // Yatay dağılım çarpanı (isteğe göre artırılabilir)

                    // Rastgelelik ekleyerek doğal görünmesini sağla
