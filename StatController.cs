...
public class StatController : MonoBehaviour, IDamageable
{
    ...
    void OnDisable()
    {
        // Debug.Log("StatController deaktif edildi! Kapatan yer burası olabilir.", tthis);
    }
    ...
    void Update()
    {
        ...
        if (checkMoney() > 250)
        {
           HintManager.Instance.ShowHint("hint_buyTurret", false, 5f);
    ...
    {
        // 0. HASAR ÇEŞİTLİLİĞİ (Opsiyonel)
        // Eğer dışarıdan belirli bir hasar değil de aralık gelirse onu kullan
        float rawDamage = damage;
        if (maxDamage > 0)
        {
            rawDamage = Random.Range(minDamage, maxDamage);
        }

        // 1. YENİ DEFANS FORMÜLÜ (K = 100)
        float k = 100f;
        float damageMultiplier = k / (k + stats.defense);

        // %90 Sınırı (Zırh ne kadar olursa olsun en az %10 hasar içeri girer)
        damageMultiplier = Mathf.Clamp(damageMultiplier, 0.1f, 1.0f);

        float damageAfterDefense = rawDamage * damageMultiplier;
        ...
        if (stats.armor > 0 && damageAfterDefense > 0)
        {
            ...
            {
                damageAfterDefense -= stats.armor;
                stats.armor = 0;
            }
        }
    ...
    {
        stats.currentHealth = Mathf.Min(stats.stamina + amount);
