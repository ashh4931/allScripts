...
public class NewMagneticNPC : NewBaseEnemyAI
{
    ...
    protected override void HandleMovement()
    {
        ...
        {
            // 1. Çok uzaktaysa oyuncuya doğru yaklaş
            rb.linearVelocity = direction * magData.moveSpeed;
        }
        else if (distanceToPlayer < magData.retreatDistance)
        {
            // 2. Oyuncu dibine girerse geri geri kaç
            rb.linearVelocity = -direction * magData.moveSpeed * 0.8f;
        }
        else
        {
            // 3. İdeal mesafede durmak yerine etrafında çember çiz (Orbit)
            // direction vektörüne 90 derece dik bir vektör oluşturuyoruz
            Vector2 orbitDirection = new Vector2(-direction.y, direction.x);

            // Kendi normal hızının yarısıyla oyuncunun etrafında süzülür
            rb.linearVelocity = orbitDirection * (magData.moveSpeed * 0.5f);
        }
    }
