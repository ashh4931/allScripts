   ...
   void Update()
    {
        if (GetComponent<weapon>().isEquipped)
        {
            ...
            // ATEŞ ETME MANTIĞI
            if (Input.GetMouseButtonDown(0) && Time.time >= nextFireTime && !isReloading && currentAmmo > 0)
            {
                nextFireTime = Time.time + 1f / fireRate;
                Attack();
            }
            // RELOAD MANTIĞI (GÜNCELLENDİ)
            else if (!isReloading && (currentAmmo <= 0 || (Input.GetKeyDown(KeyCode.R) && currentAmmo < maxAmmo)))
            {
