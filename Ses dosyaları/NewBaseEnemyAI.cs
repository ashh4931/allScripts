...
public abstract class NewBaseEnemyAI : MonoBehaviour, IDamageable
{
    ...
    protected void DropLoot()
    {
        ...
        foreach (EnemyLoot loot in baseData.lootTable)
        {
            ...
            {
                // Düşman merkezinde eşyayı yarat
                ...
                // --- SAÇILMA (PATLAMA) FİZİĞİ ---
                Rigidbody2D rbItem = droppedItem.GetComponent<Rigidbody2D>();
                if (rbItem != null)
                {
                    // 360 derece rastgele bir yön
                    Vector2 randomDirection = Random.insideUnitCircle.normalized;

                    // Fırlama hızı (Biraz daha yüksek tuttuk)
                    float burstSpeed = Random.Range(6f, 12f);

                    // AddForce yerine hızı (velocity) DİREKT veriyoruz. Bu anında tepki verir!
                    rbItem.linearVelocity = randomDirection * burstSpeed;
                }
                else
                {
                    Debug.LogWarning($"Loot objesinde ({droppedItem.name}) Rigidbody2D yok! Saçılma efekti çalışamaz.");
                }
            }
        }
    }
