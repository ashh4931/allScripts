...
public class NewSnakeAI : NewBaseEnemyAI
{
    ...
    public void OnSegmentDestroyed(Transform segment)
    {
        ...
        // Sadece bu yılanın yerel hız değişkenini artır (Data bozulmaz, eski bug geri dönmez!)
        currentMoveSpeed *= snakeData.enrageSpeedMultiplier;

        // 🔴 YENİ DÜZELTME: "Enrage Hafızası"
        // Eğer yılan şu an zaman yırtığının içindeyken (yavaşlamışken) parça kaybederse,
        // Yırtık kapandığında bu hız bonusunu unutmaması için orijinal hızı da güncelliyoruz!
        if (isTimeSlowed)
        {
            originalMoveSpeed *= snakeData.enrageSpeedMultiplier;
        }
    }
