...
public class gun6_script : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (weaponComponent != null && weaponComponent.isEquipped)
        {
            ...
            // --- ATEŞ ETME MANTIĞI (GetKey ile Seri Ateş) ---
            if (InputManager.GetKey("Attack") && Time.time >= nextFireTime && !isReloading)
            {
                if (currentAmmo > 0)
                {
                ...
                {
                    // Mermi bittiyse ve hala ateşe basılıyorsa otomatik reload başlat
