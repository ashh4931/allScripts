...
public class PlayerShieldController : MonoBehaviour
{
    ...
    {
        // 1. Kalkanı Açma Şartları (Tuşa basıldığı an)
        if (InputManager.GetKeyDown("OpenShield"))
        {
        ...
        // 2. Kalkanı Kapatma Şartları (Tuş bırakıldığı an VEYA kalkan aktifken tuş basılı değilse)
        if (isShieldActive && !InputManager.GetKey("OpenShield"))
        {
            DeactivateShield();
        }
    }
