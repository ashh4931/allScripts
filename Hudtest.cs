...
public class Hudtest : MonoBehaviour
{
   ...
   void Update()
    {
        // --- KRİTİK EKLEME: Oyuncu henüz bulunamadıysa aramaya devam et ---
        if (playerStats == null)
        {
            ...
            if (playerObj != null)
            {
                ...
                // Başlangıç değerlerini bir kez set et
                if (healthBar != null) healthBarOrigin = healthBar.transform.localPosition;
                if (staminaBar != null) staminaBarOrigin = staminaBar.transform.localPosition;
            }
            return; // Oyuncu bulunana kadar Update'in geri kalanını çalıştırma
        }

        // Oyuncu bulunduktan sonra aşağıdaki normal akış devam eder
        UpdateHealth();
        UpdateMana();
        updateStamina();
        UpdateStaminaShake();
